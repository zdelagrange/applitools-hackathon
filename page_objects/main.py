class MainPage(object):
    ...
    def get_element_compare_expenses(self):
        return self.driver.find_element_by_css_selector(self.LOCATORS['compare_expenses'])
    ...
    def get_elements_table_rows(self):
        return self.driver.find_elements_by_css_selector(self.LOCATORS['recent_transaction_rows'])

    def get_row_data(self):
        rows = []
        for row in self.get_elements_table_rows():
            data = {}
            data['status'] = row.find_element_by_css_selector('td:nth-of-type(1)>span:nth-of-type('
                                                             '2)').text
            data['date'] = row.find_element_by_css_selector('td:nth-of-type(2)>span:nth-of-type('
                                                           '1)').text
            data['description'] = row.find_element_by_css_selector('td:nth-of-type(3)>span').text
            data['category'] = row.find_element_by_css_selector('td:nth-of-type(4)>a').text
    ...
    def get_amounts_float(self):
        return [self.get_number_from_amount_row(ele.text) for ele in self.get_elements_amounts()]
